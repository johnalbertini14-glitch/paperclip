...
suite("workspaceOperations issue reference guard", () => {
  ...
  async function seedTwoCompanyScenario() {
    ...
    await db.insert(companies).values(
      { id: companyA, name: "AlphaCo", issuePrefix: `ACA${companyA.replace(/-/g, "").slice(0, 3).toUpperCase()}`, status: "active", createdAt: new Date(), updatedAt: new Date() },
    );
    await db.insert(companies).values(
      { id: companyB, name: "ZetaInc", issuePrefix: `ZIN${companyB.replace(/-/g, "").slice(0, 3).toUpperCase()}`, status: "active", createdAt: new Date(), updatedAt: new Date() },
    );
  ...
  it("accepts valid same-company issue UUID (guard validates; issueId not persisted per schema contract)", async () => {
    const companyId = randomUUID();
    const projectId = randomUUID();
    const issueId = randomUUID();
    ...
    // The guard validated the same-company issue reference before the INSERT.
    // The schema does not persist issueId — the shared type has no issueId field,
    // so the returned object omits it entirely.
    ...
    // Explicitly verify issueId is absent (not persisted in schema).
    expect("issueId" in result).toBe(false);
